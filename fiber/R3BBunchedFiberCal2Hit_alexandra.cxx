...
#include "/u/schulte/software/R3BRoot/r3bdata/tofData/R3BTofdHitData.h"
...
// Test:
    ...
    fh_Test = new TH2F(chistName.Data(), chistTitle.Data(), 512,0,512, 100, 0., 100.);
   ...
   //  cout<<"multihit"<<endl;
    ...
    for (size_t j = 0; j < cal_num; ++j)
    {
        ...
        if (cur_cal->IsTrailing())
        {
          ...
          if(fName == "Fi10" && fnEvents == 17409 && side_i == 0) summmpt += 1; 
          if(fName == "Fi10" && fnEvents == 17409 && side_i == 1 && ch_i == 0) summsm1 += 1; 
          if(fName == "Fi10" && fnEvents == 17409 && side_i == 1 && ch_i == 1) summsm2 += 1; 
          
         if(fName == "Fi10" && fnEvents == 17409 ) cout<<"Input: "<<side_i<<"; "<<ch_i+1<<", "<<lead->GetTime_ns()<<"; "<<tot<<endl;
            
            if (tot < 1000)
            {
                // if (side_i==0) tot -= 9.;//taken out for the moment MH

               // channel.tot_list.push_back(ToT(lead, cur_cal, tot)); //-> this lead to: t1>t2>t3....; we need t1<t2<t3....
                 channel.tot_list.push_front(ToT(lead, cur_cal, tot));
    ...
    if(fName == "Fi10" && fnEvents == 17409 ) cout<<"Summs: "<<summmpt<<", "<<summsm1<<", "<<summsm2<<endl;
...
//MH                     auto fiber_id = mapmt_tot.lead->GetChannel(); 
...
// MH                    if (fIsCalibrator)
                        ...
                        if((t_spmt - t_mapmt) > 90. || (t_spmt - t_mapmt) < 60.) fh_Test->Fill(fiber_id,tot_mapmt);

// MH                    LOG(DEBUG) << fName << " tS " << tot_spmt << " tM: " << t_mapmt << " totS: " << tot_spmt
// MH                               << " totM: " << tot_mapmt << endl;

                    // Apply calibration.
                    Double_t gainMA =  10. ;//gain_temp[fiber_id-1]; //10.;
                    Double_t gainS = 10.;
                    Double_t offset1 = 0.;
                    Double_t offset2 = 0.;
                    Double_t tsync = 0.; //tsync_temp[fiber_id-1];//0.;
...
// MH                    fh_dt_Fib->Fill(fiber_id, t_spmt - t_mapmt);
                  ...
                  if((t_spmt - t_mapmt) < 90. && (t_spmt - t_mapmt) > 60.)  fh_ToT_MA_Fib->Fill(fiber_id, tot_mapmt);
                    if (s_mult > 0)
                    {
...
//MH                    Int_t numFibs = fSubNum * fChPerSub[0];
...
// MH                            R3BBunchedFiberHitData(0, x, y, eloss, t, fiber_id, t_mapmt, t_spmt, tot_mapmt, tot_spmt);
     ...
     // tsync  
		if(fIsTsync){
			for (UInt_t i = 1; i <= max; i++)
			{
				...
				cout << "MA fiber: " << fName<<", "<<i << " tsync: " <<  proj->GetBinCenter(proj->GetMaximumBin()) << ", "<<par->GetSync()<<
			", "<<par->GetGainMA()<<endl;
			}
		}
        // time offset
