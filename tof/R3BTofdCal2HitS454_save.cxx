...
//std::cout << "Top: " << top->GetDetectorId() << ' ' << top->GetSideId() << ' ' << top->GetBarId() << ' ' << top_trig_i << ' ' << top_trig->GetTimeLeading_ns() << std::endl;
//std::cout << "Bot: " << bot->GetDetectorId() << ' ' << bot->GetSideId() << ' ' << bot->GetBarId() << ' ' << bot_trig_i << ' ' << bot_trig->GetTimeLeading_ns() << std::endl;

// Shift the cyclic difference window by half a window-length and move it back,
// this way the trigger time will be at 0.
            auto top_ns = fmod(top->GetTimeLeading_ns() - top_trig->GetTimeLeading_ns() + c_range_ns + c_range_ns/2, c_range_ns) - c_range_ns/2;
            auto bot_ns = fmod(bot->GetTimeLeading_ns() - bot_trig->GetTimeLeading_ns() + c_range_ns + c_range_ns/2, c_range_ns) - c_range_ns/2;
//std::cout << top->GetTimeLeading_ns() << ' ' << top_trig->GetTimeLeading_ns() << ' ' << top_ns << std::endl;
//std::cout << bot->GetTimeLeading_ns() << ' ' << bot_trig->GetTimeLeading_ns() << ' ' << bot_ns << std::endl;
...
void R3BTofdCal2HitS454::FinishTask()
{
    ...
    std::cout << "\n\nSome statistics:\n"
              ...
              << "Good events in total             " << eventstore << " <-> "
              << singlehit << " = singlehit \n";
}
